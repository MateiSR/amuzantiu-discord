...
export default new PrefixCommand({
  ...
  run: async ({ client, message, args }) => {
    ...
    if (parseInt(args[0]) < 1 || parseInt(args[0]) > 100)
      return await message.reply("Please enter a number between 1 and 100")
    ...
    try {
      ...
      await channel.bulkDelete(messages, true).then(async () => {
        return await (message.channel as TextChannel)
          .send(`✅ **Succesfully deleted messages** *newer than 2 weeks*`)
          .then(m => setTimeout(() => m.delete(), 2500))
      })
    } catch (error) {
      return await (message.channel as TextChannel).send(
        `You cannot delete messages that are older than **14 days old**.`,
      )
    }
  },
})
