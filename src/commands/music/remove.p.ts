...
export default new PrefixCommand({
    ...
    run: async ({ client, message, args }) => {
        ...
        if (!dispatcher || !dispatcher.current) return await message.reply({ embeds: [client.util.embed("Bot is not playing music", Colors.Red, "Please play a song and try again")] });
        // check if position is valid
        const position = parseInt(args[0]) || 0;
        if (position < 1 || position > dispatcher.queue.length) return await message.reply({ embeds: [client.util.embed("Invalid position", Colors.Red, "Please enter a valid position and try again")] });

        // remove song from queue
        const removed = dispatcher.queue.splice(position - 1, 1);
        await message.reply({ embeds: [client.util.embed("Song removed", Colors.Green, `Removed [${removed[0].info.title}](${removed[0].info.uri}) from the queue`)] });
