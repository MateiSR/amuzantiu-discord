...
export default new PrefixCommand({
    ...
    run: async ({ client, message, args }) => {
        ...
        // Check if plaing
        if (dispatcher.player.paused) return await message.reply({ embeds: [client.util.embed("Music is paused", Colors.Red, "Please resume the music and try again")] });
        ...
        if (!args[0] ||  parseInt(args[0]) < 0 || parseInt(args[0]) > 500) return await message.reply({ embeds: [client.util.embed("Invalid volume", Colors.Red, "Please enter a valid volume **between 1-500**")] });
        // Get volume
        const volume = parseInt(args[0]) || 100;
        // Set volume
        dispatcher.player.setVolume(volume / 100);
        // Send success message
        await message.reply({ embeds: [client.util.embed("Volume set", Colors.Green, `Set volume to **${volume}%**`)] });
}});