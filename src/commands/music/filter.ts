...
export default new Command({
    ...
    async run({ client, interaction, args }) {
        ...
        if (filter === "off") {
            ...
            return await interaction.followUp({ embeds: [client.util.embed("Filters removed", Colors.Green, "Previously applied filter(s) has been removed")] });
        }

        // set filter shoukaku
        try {
        dispatcher.setFilter(filter);
        await interaction.followUp({ embeds: [client.util.embed("Filter set", Colors.Green, `Filter set to ${filter}`)] });
        } catch (e) {
            return await interaction.followUp({ embeds: [client.util.embed("Invalid filter", Colors.Red, "Please provide a valid filter")] });
        }
