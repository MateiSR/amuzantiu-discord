...
export default new PrefixCommand({
    ...
    run: async ({ client, message, args }) => {
        ...
        if (!dispatcher  || !dispatcher.player.connection) return await message.reply({ embeds: [client.util.embed("No song is playing", Colors.Red, "Please play a song first")] });
        ...
        try {
        dispatcher.stop();
        return await message.reply({ embeds: [client.util.embed("Stopped - Leaving..", Colors.Green, `Leaving voice channel`)] });
        } catch (error) {
