...
export default new Command({
    ...
    run: async ({ client, interaction, args }) => {
        ...
        if (dispatcher.player.paused) {
            dispatcher.resume();
            return await interaction.followUp({ embeds: [client.util.embed("Resumed", Colors.Green, `Resumed playing [${dispatcher.current.info.title}](${dispatcher.current.info.uri})`)] });
        }
        else {
            // Pause the player
            dispatcher.pause();
            return await interaction.followUp({ embeds: [client.util.embed("Paused", Colors.Green, `Paused playing [${dispatcher.current.info.title}](${dispatcher.current.info.uri})`)] });
        }
