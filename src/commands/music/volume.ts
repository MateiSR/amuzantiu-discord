...
export default new Command({
    ...
    run: async ({ client, interaction, args }) => {
        ...
        // Check if plaing
        if (dispatcher.player.paused) return await interaction.followUp({ embeds: [client.util.embed("Music is paused", Colors.Red, "Please resume the music and try again")] });
        ...
        await interaction.followUp({ embeds: [client.util.embed("Volume set", Colors.Green, `Set volume to **${volume}%**`)] });
}});