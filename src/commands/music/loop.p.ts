...
export default new PrefixCommand({
    ...
    run: async ({ client, message, args }) => {
        ...
        if (!mode) {
            dispatcher.loop = dispatcher.loop === "none" ? "track" : dispatcher.loop === "track" ? "queue" : "none";
            // send embed
            return await message.reply({ embeds: [client.util.embed("Loop mode set", Colors.Green, `Loop mode set to ${dispatcher.loop}`)] });
        }

        // check if mode is valid
        if (!["none", "track", "queue"].includes(mode)) return await message.reply({ embeds: [client.util.embed("Invalid mode", Colors.Red, "Please provide a valid mode `(none, track, queue)`")] });

        // set mode
        dispatcher.loop = mode;

        // send embed
        return await message.reply({ embeds: [client.util.embed("Loop mode set", Colors.Green, `Loop mode set to ${mode}`)] });
