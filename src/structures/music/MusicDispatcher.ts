...
export default class MusicDispatcher {
	...
	constructor(options: DispatcherOptions) {
		...
		this.player
			...
			.on("exception", (exception: TrackExceptionEvent) =>
				this.onTrackException(exception),
			);
	}
	...
	onTrackStart = async () => {
		...
		await channel.send({
			embeds: [
				client.util
					...
					.setThumbnail(
						client.manager.util.getYouTubeThumbnail(
					...
					.addFields([
						{
							name: "Source",
							value:
								this.current.info.sourceName == "youtube"
									? `[YouTube](${this.current.info.uri})`
									: this.current.info.sourceName,
							inline: true,
						},
						{
							name: "Duration",
							value: this.current.info.isStream
								? "LIVE"
								: client.util.formatTime(this.current.info.length),
							inline: true,
						},
						{
							name: "Requested by",
		...
		this.client.logger.debug(
			`Now playing ${this.current.info.title} in ${this.guild.name} (${this.guild.id}) --- Requested by ${requester.user.tag}`,
		);
	};
	...
	onTrackException = async (exception: TrackExceptionEvent) => {
		...
		await channel.send({
			embeds: [
				client.util.embed(
					"!   Exception encountered while trying to play track",
					Colors.Red,
					`[${this.current.info.title}](${this.current.info.uri})`,
				),
			],
	...
	async setFilter(filterOption: string) {
		if (!this.player) {
			client.logger.debug("No player found");
			return;
		}
	...
	destroy() {
		...
		/*const guildConnection = client.shoukaku.connections.get(this.guild.id);
    guildConnection.disconnect();*/
		client.shoukaku.leaveVoiceChannel(this.guild.id);
