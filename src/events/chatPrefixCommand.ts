...
export default new Event("messageCreate", async (message) => {
  ...
  if (!process.env.prefix)
    return console.log("Please set the prefix in the .env file");
  ...
  if (!command) {
    if (slashCommand)
      return message.reply(
        "This command is only available as a **slash command**, please **try again**",
      );
    else return;
  }
  ...
  if (command.defaultMemberPermissions) {
    ...
    if (
      !channel
        ...
        .has(command.defaultMemberPermissions)
    )
      return message.reply(
    ...
    if (
      ...
      return message.reply(
        "The bot does not have enough permissions to use this command.",
      );
  }
